...
import com.yugabyte.cdcsdk.engine.spi.OffsetCommitPolicy;
...
public class OffsetCommitPolicyTest {
    ...
    public void shouldCommitPeriodically() {
        // 10 hours
        OffsetCommitPolicy policy = OffsetCommitPolicy.periodic(Configuration.create().with(MTEngine.OFFSET_FLUSH_INTERVAL_MS, 10 * 60 * 60 * 1000).build());
    ...
    public void shouldCommitPeriodicallyWithProperties() {
        // 10 hours
        final Properties props = Configuration.create().with(MTEngine.OFFSET_FLUSH_INTERVAL_MS, 10 * 60 * 60 * 1000).build().asProperties();
