...
public class Transformations implements Closeable {
    ...
    public Transformations(Configuration config) {
        this.config = config;
        final String transformationList = config.getString(MTEngine.TRANSFORMS);
        ...
        for (String transfName : transformationList.split(",")) {
            ...
            LOGGER.trace("Added transform {}", transfName);
        }
        LOGGER.info("Added {} transformations.", transforms.size());
    }

    private String transformationConfigNamespace(final String name) {
        return MTEngine.TRANSFORMS.name() + "." + name;
    }
    ...
    private Transformation<SourceRecord> getTransformation(String name) {
        ...
        try {
            transformation = config.getInstance(MTEngine.TRANSFORMS.name() + "." + name + TYPE_SUFFIX, Transformation.class);
        }
        catch (Exception e) {
